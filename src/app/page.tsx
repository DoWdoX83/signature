...
async function buildAxaSecondWithAdvisor(base64: string, advisorSigRef: React.MutableRefObject<SignaturePad | null>, docType: string): Promise<Uint8Array> {
  ...
  if (advisorSigRef.current && !(advisorSigRef.current as any).isEmpty()) {
    ...
    last2.drawImage(img, { x: sigX, y: sigY, width: img.width * 0.3, height: img.height * 0.3, opacity: 1 });
  }
                ...
                {showHint && (
                  <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
                    <svg width="60%" height="60" viewBox="0 0 300 60">
                      <path d="M5 40 C 40 10, 80 60, 120 30 S 200 40, 295 20" fill="none" stroke="#000000" strokeWidth="3" style={{ animation: "stroke 2.6s ease-in-out infinite" }} />
                    </svg>
                  </div>
                )}
                <SignaturePad
